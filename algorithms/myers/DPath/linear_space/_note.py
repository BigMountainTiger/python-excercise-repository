...
# This implementaton seems not favor deletion over insertion
# - delete before insert more natural for human to read
        ...
        for h in range(0, (L//2+(L % 2 != 0))+1):
            for r in range(0, 2):
                ...
                for k in range(-(h-2*max(0, h-M)), h-2*max(0, h-N)+1, 2):
                    a = c[(k+1) % Z] if (k == -h or k != h and c[(k-1) %
                                                                 Z] < c[(k+1) % Z]) else c[(k-1) % Z]+1
                    b = a-k
                    s, t = a, b
                    ...
                    if L % 2 == o and z >= -(h-o) and z <= h-o and c[k % Z]+d[z % Z] >= N:
                        D, x, y, u, v = (
                            2*h-1, s, t, a, b) if o == 1 else (2*h, N-a, M-b, N-s, M-t)
